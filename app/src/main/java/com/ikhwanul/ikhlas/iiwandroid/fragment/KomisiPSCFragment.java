...
public class KomisiPSCFragment extends AppFragment implements iPresenterResponse, SearchView.OnQueryTextListener {

    private OnFragmentInteractionListener mListener;
    private SwipeRefreshLayout mSwipeRefreshLayout;
    private RecyclerView rvKomisiPSC;
    private KomisiPSCAdapter mAdapter;
    private SearchView svData;
    private Presenter mPresenter;
    private List<KomisiPSC> dataKomisiPSC;
    ...
    public KomisiPSCFragment() {
        super();
        this.res_layout = R.layout.fragment_komisi_psc;
    }

    public static KomisiPSCFragment newInstance() {
        KomisiPSCFragment fragment = new KomisiPSCFragment();
        return fragment;
    }
    ...
    private void initObject() {
        ...
        mPresenter.getKomisiPSC(Integer.valueOf(dataUser.id_perwakilan), progressDialog, true);
    ...
    protected void initView(){
        ...
        rvKomisiPSC = (RecyclerView) findViewById(R.id.rv_komisi_psc);
        rvKomisiPSC.setHasFixedSize(true);
    ...
    private void initEvent(){
        mSwipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                mPresenter.getKomisiPSC(Integer.valueOf(dataUser.id_perwakilan), progressDialog, false);
            }
        });
    }
    ...
    public void doSuccess(ApiResponse response, String tag) {
        if (tag.equals(Presenter.RES_GET_DATA_KOMISI_PSC)){
            dataKomisiPSC = ((KomisiPSCResponse)response).komisi;
            if (dataKomisiPSC.size() > 0){
                mAdapter = new KomisiPSCAdapter(getContext(), dataKomisiPSC);
                rvKomisiPSC.setLayoutManager(new GridLayoutManager(getContext(), 1));
                rvKomisiPSC.setAdapter(mAdapter);
                rvKomisiPSC.setVisibility(View.VISIBLE);
                layoutInsufficient.setVisibility(View.GONE);
            }else{
                rvKomisiPSC.setVisibility(View.GONE);
